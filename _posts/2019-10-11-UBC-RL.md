...
how? just run $p_{\pi_\theta}(a_t \mid o_t)$

but need labels $a_t$ !

1. train $\pi_{\theta}(a_t \mid o_t)$ from human data $\mathcal{D}={o_1,a_1,...,o_N,a_N}$ 
2. run $\pi_\theta(a_t \mid o_t)$ to get dataset $\mathcal{D}_\pi = {o_1,...,o_M}$
...
1. sample ${\tau^i}$ from $\pi_\theta(s_t \mid s_t)$ (run the policy)
2. $\Delta_\theta J(\theta)\approx\sum_{i}^N\left(\sum_{t} \Delta_\theta \log \pi_\theta (a_t \mid s_t) \right)\left(\sum_{t} r(s_t,a_t)\right)$
...
4. $\Delta_\theta J(\theta)\approx \sum_i \Delta_\theta \log \pi_\theta (a_{i} \mid s_{i})\hat{A}^\pi(s_i,a_i)$ 
...
1. take action $a\sim\pi_\theta(a \mid s)$, get $(s,a,s',r)$
...
4. $\Delta_\theta J(\theta)\approx \Delta_\theta \log \pi_\theta (a \mid s)\hat{A}^\pi(s,a)$ 
5. $\theta \gets \theta+\alpha\Delta_\theta J(\theta)$
...
> this at least as good as any $a_t \sim \pi(a_t, \mid s_t)$
...
assume we know $p(s' \mid s,a)$ and s and a are both discrete (and small)
...
1. set $y_i \gets \max_{a_i}(r(s_i \mid a_i)+\gamma E[V_\phi(s'_i)])$
...
3. set $\phi \gets \arg min_\phi \frac{1}{2}\sum_i\ \mid Q_\phi(s_i,a_i)-y_i\ \mid ^2$ repeat step 2,3 k times and then return step 1
...
1. save target network parameters: $\phi' \gets \phi$
 ...
 1. sample a batch $(s_i,a_i,s_i',r_i)$ from $\mathcal{B} $ 
 2. $\phi \gets \arg min_\phi \frac{1}{2}\sum_i\ \mid Q_\phi(s_i,a_i)- [r(s_i,a_i) +\gamma \max_{a_i'}Q_{\phi'}(s_i',a_i')]\ \mid ^2$ , do 2,3 k times
...
1. sample $\{\tau^i\}$ from $\pi_\theta(s_t \mid s_t)$ (run the policy)
2. $\Delta_\theta J(\theta)\approx\sum_{i}\left(\sum_{t=1}^T \Delta_\theta \log \pi_\theta (a_t^i \mid s_t^i) \left(\sum_{t'=t}^T r(s_{t'},a_{t'})\right)\right)$
3. $\theta \gets\theta+\alpha\Delta_\theta J(\theta)$
...
$\pi_{\theta'}$ if close to $\pi_\theta$ if $ \mid \pi_{\theta'(a_t \mid s_t)}-\pi_\theta(a_t \mid s_t) \mid \le\epsilon$ for all $s_t$ 

$ \mid p_{\theta'}(s_t)-p_\theta(s_t) \mid \le 2\epsilon t$
...
A more convenient bound is using KL divergence: $ \mid \pi_{\theta'(a_t \mid s_t)}-\pi_\theta(a_t \mid s_t) \mid \le \sqrt{\frac{1}{2}D_{KL}(\pi_{\theta'}(a_t \mid s_t)\ \mid \pi(a_t \mid s_t))}$

$\Rightarrow D_{KL}(\pi_{\theta'}(a_t \mid s_t)\ \mid \pi(a_t \mid s_t)$ bounds state marginal difference, where
...
2. $\lambda \gets + \alpha(D_{KL}(\pi_{\theta'}(a_t \mid s_t)\ \mid \pi(a_t \mid s_t))-\epsilon)$
...
In model-free RL, we do not know $p(s_{t+1} \mid s_t,a_t)$. 
...
>
> (or $p(s_{t+1} \mid s_t,a_t)$ in stochastic case)

model-based reinforcement learning **version 0.5**:

1. run base policy $\pi_0(a_t \mid s_t)$ (e.g., random policy) to collect $\mathcal{D}={(s,a,s')_i}$
2. learning dynamics model $f(s,a)$ to minimize $\sum_i\ \mid f(s_i,a_i)-s_i'\ \mid ^2$
...
2. at each time step $t$, sample $s_{t+1}\sim p(s_{t+1} \mid s_t,a_t,\theta)$
...
learn *approximate* posterior $q_\psi(s_t \mid o_{1:t},a_{1:t})$ 

other choices:

- $q_\psi(s_t,s_{t+1} \mid o_{1:t},a_{1:t})$
- $q_\psi(s_t \mid o_t)$

here we only estimate $q_\psi(s_t \mid o_t)$

assume that $q_\psi(s_t \mid o_t)$ is *deterministic*
...
1. run base policy $\pi_0(o_t \mid a_t)$ (e.g., random policy) to collect $\mathcal{D}={(o,a,o')_i}$
2. learning $p_\psi(s_{t+1} \mid s_t,a_t), p_\psi(r_t \mid s_t), p(o_t \mid s_t), g_\psi(o_t)$
...
directly learn $p(o_{t+1} \mid o_t,a_t)$ 
...
1. run base policy $\pi_0(a_t \mid s_t)$ (e.g., random policy) to collect $\mathcal{D}={(s,a,s')_i}$
2. learning dynamics model $f(s,a)$ to minimize $\sum_i\ \mid f(s_i,a_i)-s_i'\ \mid ^2$
3. back-propagate through $f(s,a)$ into the policy to optimize $\pi_\theta(a_t \mid s_t)$
4. run $\pi_\theta (a_t \mid s_t)$, appending the visited tuples $(s,a,s')$ to $\mathcal{D}$ , repeat steps 2~4
...
- supervised objective for $\pi_\theta(u_t \mid x_t)$
...
$$
\min_{p,\theta}E_{\tau\sim p(\tau)}[{c(\tau)}]\:\:\text{s.t.}\:\:p(u_t \mid x_t)=\pi_\theta(u_t \mid o_t)\
$$
...
3. execute action $s_t\sim\pi(a_t \mid s_t)$ (not MCTS action!). repeat 1~3 N times
...
1. train $\pi_\theta(u_t \mid o_t)$ from labeled data $\mathcal{D}={o_1,u_1,...,o_N,u_N}$
2. run $\hat{\pi}(u_t \mid o_t)$ to get dataset $\mathcal{D}_\pi={o_1,...,o_M}$
3. Ask computer to label $\mathcal{D_\pi}$ with actions $u_t$
4. Aggregate: $\mathcal{D}\gets\mathcal{D}\cup\mathcal{D}_\pi$

**Simple** stochastic policy: $\hat{\pi}(u_t \mid x_t)=\mathcal{N}(K_tx_t+k_t, \Sigma_{u_t})$
...
the model: $p(x)=\int p(x \mid z)p(z)dz$

maximum likelihood fit: $\theta\gets\arg\max_\theta \frac{1}{N} \sum_i \log \left(\int p(x \mid z)p(z)dz\right)$

Estimating the log-likelihood

alternative: *expected* log-likelihood: $\theta\gets\arg\max_\theta \frac{1}{N} \sum_i E_{z\sim p(z \mid x_i)} \log p_\theta(x_i,z)$

##### The variational approximation

approximate $p(z \mid x_i)$ with $q_i(z)=\mathcal{N}(\mu_i,\sigma_i)$ 
...
KL Divergence: $D_{KL}(q \mid \mid p)=E_{x \sim q(x)}\left[\log \frac{q(x)}{p(x)}\right]=E_{x \sim q(x)}[\log q(x)]- E_{x \sim q(x)}[\log p(x)] =-E_{x \sim q(x)}[\log p(x)]- \mathcal{H}(q)$

further analysis

Unable to render expression.
$$
\log p(x_i) = E_{z \sim q_i(z)}[\log p(x_i \mid z)+\log p(z)]+ \mathcal{H}(q_i)=\mathcal{L}_i(q,p_i)
$$

so what makes a good $q_i(z)$? 

intuition: $q_i(z)$ should approximate $p(z \mid x_i)$
...
> So this also prove that $\mathcal{L}_i(p,q_i)$ is low bound, and the KL Divergence is the bound gap, so when $q_i(x)$ close to $p(z \mid x_i)$, the bound will become tight.
>
...
​	$\Delta_\theta \mathcal{L}(p,q_i)\approx\Delta_\theta \log p_\theta(x_i \mid z)$
...
what if we learn a *network* $q_i(z)=q(z \mid x_i)\approx p(z \mid x_i)$ ?

so we have two network: $p_\theta(x \mid z)$ and $q_\phi(z \mid x)$
...
​	calculate $\mathcal{L}(q_\theta(x_i \mid z),q_\phi(z \mid x_i))$

​	sample $z \sim q_\phi(z \mid x_i)$
...
- policy gradient $J(\phi)\approx \frac{1}{M}\sum_j\Delta_\phi \log q_\phi(z_j \mid x_i)r(x_i,z_j)$
...
>
> we want to represent the distribution of a object(by neural network) and make it has ability to generalize the feature of the object (multi-modal). To achieve this, the random variables($z$) are used as input of model, but what kind of random variances can achieve this? the mathematic proof shows that the distribution of random variance should approximate $p(z \mid x_i)$, this is kind of compress sensing, where $z$ is the latent variable. Finally, this is used to do variational auto encoder (VAE).
>
...
with temperature: $\pi(a_t \mid s_t)=\exp(\frac{1}{\alpha}Q_t(s_t,a_t)-\frac{1}{\alpha}V_t(s_t))=\exp(\frac{1}{\alpha}A_t(s_t,a_t))$ if $\alpha$ is near zero, the max action will dominate, so it's more like deterministic, and $\alpha$ near 1 is more stochastic.
...
marginalizing and conditioning, we get: $p(a_t \mid s_t,\mathcal{O}_{1:T})$ (the policy)

> "given that you obtained high reward, what was your action probability?"

marginalizing and conditioning, we get: $p(s_{t+1} \mid s_t,a_t,\mathcal{O}_{1:T})\ne p(s_{t+1} \mid s_t,a_t)$ 
...
find anther distribution $q(s_{1:T},a_{1:T})$ that is close to $p(s_{1:T},a_{1:T} \mid \mathcal{O}_{1:T})$ but has dynamics $p(s_{t+1} \mid s_t,a_t)$
...
\log p(\mathcal{O}_{1:T}) &\ge E_{s_{1:T},a_{1:T}\sim q}[\log p(s_1)+\sum_{t=1}^T\log p(s_{t+1} \mid s_t,a_t)+\sum_{t=1}^T\log p(\mathcal{O}_t \mid s_t,a_t)\\-\log p(s_1)-&\sum_{t=1}^T\log p(s_{t+1} \mid s_t,a_t)-\sum_{t=1}^T\log q(s_t \mid s_t)]\\
...
$\pi(a \mid s)=\exp(Q_\phi(s,a)-V(s))$ optimizes $\sum_{\pi(s_t,a_t)}[r(s_t,a_t)]+E_{\pi(s_t)}[\mathcal{H}(\pi(a_t \mid s_t))]$

**intuition:** $\pi(a \mid s)\propto \exp(Q_\phi(s,a))$ when $\pi$ minimizes $D_{KL}(\pi(a \mid s) \mid \mid \frac{1}{Z}\exp(Q(s,a))$
...
(sometimes) transitions $p(s' \mid s,a)$
...
… and then use it to learn $\pi^*(a \mid s)$

#### Learning the optimality variable

$p(\mathcal{O}_t \mid s_t,a_t,\psi)=\exp(r_\psi(s_t,a_t))$

$p(\tau \mid \mathcal{O}_{1:T},\psi) \propto p(\tau)\exp\left(\sum_t r_\psi(s_t,a_t)\right)$
...
$ p(s_t,a_t \mid \mathcal{O}_{1:T},\psi)=p(a_t \mid s_t,\mathcal{O}_{1:T},\psi)p(s_t \mid \mathcal{O}_{1:T},\psi)$
...
instead of learn the $p(a_t \mid s_t, \mathcal{O}_{1:T},\psi)$using any max-ant RL algorithm to converge then run this policy to sample ${\tau_j}$, can we just run one or several gradient step and then sample? 
...
let $\mathcal{H}(\hat{p}(z) \mid y)$ be the entropy of our $z$ estimate after observation $y$

Information gain is

$$
IG(z,y)=E_y[\mathcal{H}(\hat{p}(z))-\mathcal(\hat{p}(z) \mid y)]\\
IG(z,y \mid a)=E_y[\mathcal{H}(\hat{p}(z))-\mathcal(\hat{p}(z) \mid y) \mid a]
$$
...
variational inference:
 ...
 IG can be equivalently written as $D_{KL}(p(z \mid y) \mid \mid p(z))$

learn about *transitions* $p_\theta(s_{t+1} \mid s_t,a_t): z=\theta$ 

$y = (s_t,a_t,s_{t+1})$ $D_{KL}(p(\theta \mid h,s_t,a_t,s_{t+1}) \mid \mid p(\theta \mid h))$ where h is the history of all prior transitions

intuition: a transition is more informative if it causes belief over $\theta$ to change

idea: use variational inference to estimate $q(\theta \mid \phi)\approx p(\theta \mid h)$

given new transition $(s,a,s')$, update $\phi$ to get $\phi'$

and use $D_{KL}(q(\theta \mid \phi) \mid \mid q(\theta \mid \phi))$ as approximate bonus
...
2. initialize $\pi_\theta$ as $\max_\theta \sum_i \log \pi_\theta(a_i \mid s_i)$
...
How to pick $\pi_{new}(a \mid s)$?

option 1: stay close to $\beta$

- e.g. $D_{KL}(\pi_{new}(. \mid s) \mid \mid \beta(. \mid s))\le \epsilon$
...
imitation objective: $\sum_{(s,a) \in \mathcal{D}_{demo}}\log \pi_\theta(a \mid s)$

RL objective: $E_{\pi_\theta}[r(s,a)]$

hybrid objective: $E_{\pi_\theta} [r(s,a)]+\lambda \sum_{(s,a) \in \mathcal{D}_{demo}} \log \pi_\theta(a \mid s)$
...
empowerment: $\mathcal{I}(s_{t+1},a_t)=\mathcal{H}(s_{t+1})-\mathcal{H}(s_{t+1} \mid a_t)$

### Learning without a reward function by reaching goals

one way is giving goal states, maybe using VAE to generate goals

1. Propose goal: $z_g \sim p(z)$, $x_g \sim p_\theta(x_g \mid z_g)$
2. Attempt to reach goal using $\pi(a \mid x,x_g)$, reach $\bar{x}$
3. Use data to update $\pi$
4. Use data to update $p_\theta(x_g \mid z_g)$, $q_\phi(z_g \mid x_g)$
...
$\pi(s \mid S,G)$ trained to reach goal G as $\pi$ gets better, final state S gets close to G, 

that means $p(G \mid S)$ becomes more deterministic!

so we actually doing this

$$
\max \mathcal{H}(p(G))-\mathcal(p(G \mid S))=\max \mathcal{I}(S;G)
$$

### Learning diverse skills

$\pi(s \mid s,z)$ and $z$ is task index.
...
where $r(s,z)= \log p(z \mid s)$, rewarding states that are unlikely for other $z' \ne z$.

Here, when z is sampled, the only thing learning doing is maximize the probability of state of this policy by tune the $\pi(s \mid z)$, it turns out that different policies get different skills. Ref. Diversity is all you need (ICLR-2019)

In fact this is also goal reaching.

$$
\mathcal{I}(z,s)=H(z)-H(z \mid s)
$$

we actually maximize $H(z)$ by sampling $p(z)$ uniformly, and then minimize $H(z \mid s)$ using the algorithm.
