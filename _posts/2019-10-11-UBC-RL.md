...
find another distribution $q(s_{1:T},a_{1:T})$ that is close to $p(s_{1:T},a_{1:T} \mid \mathcal{O}_{1:T})$ but has dynamics $p(s_{t+1} \mid s_t,a_t)$

Try variational inference!

let $\mathbf{x}=\mathcal{O}_{1:T}$ and $\mathbf{z}=(s_{1:T},a_{1:T})$; find $q(\mathbf{z})$ to approximate $p(\mathbf{z} \mid \mathbf{x})$
...
- (sometimes) a transition model $p(s' \mid s,a)$  
- demonstrations $\{\tau_i\}$ drawn from $\pi^*(\tau)$

Goal: learn $r_\psi(s,a)$ and then recover $\pi^*(a \mid s)$.
...
Here the demonstrations are $\{\tau_i\}$ sampled from $\pi^*(\tau)$ and we perform maximum-likelihood learning:
...
where $\log Z$ ensures the trajectory probabilities sum to one.
