...
how? just run $p_{\pi_\theta}(a_t\mido_t)$

but need labels $a_t$ !

1. train $\pi_{\theta}(a_t\mido_t)$ from human data $\mathcal{D}={o_1,a_1,...,o_N,a_N}$ 
2. run $\pi_\theta(a_t\mido_t)$ to get dataset $\mathcal{D}_\pi = {o_1,...,o_M}$
...
$$
p_\theta(s_1,a_1,...,s_T,a_T)=p_\theta(\tau)=p(s_1)\prod_{t=1}^T\pi(a_t\mids_t)p(s_{t+1}\mids_t,a_t)\\
...
$$
V^\pi(s_t)=\sum_{t'=t}^TE_{\pi_\theta}[r(s_{t'},a_{t'})\mids_t]\\
v^\pi(s_t)=E_{s_t\sim\pi(a_t\mids_t)}[Q^\pi(s_t,a_t)]
$$
...
\pi_\theta(\tau)&=\pi_\theta(s_1,a_1,...,s_T,a_T)=p(s_1)\prod_{t=1}^T\pi(a_t\mids_t)p(s_{t+1}\mids_t,a_t)\\
\log \pi_\theta(\tau) &=\log p(s_1) + \sum_{t=1}^T \log \pi_\theta (a_t\mids_t) + \log p(s_{t+1}\mids_t,a_t)\\
&\Delta_\theta \left[\log p(s_1) + \sum_{t=1}^T \log \pi_\theta (a_t\mids_t) + \log p(s_{t+1}\mids_t,a_t)\right]= \sum_{t=1}^T \Delta_\theta \log \pi_\theta (a_t\mids_t) 
\end{align}
$$
...
&=E_{r\sim\pi_\theta}\left[\left(\sum_{t=1}^T \Delta_\theta \log \pi_\theta (a_t\mids_t) \right)\left(\sum_{t=1}^T r(s_t,a_t)\right)\right]
\end{align}
$$
...
$$
\begin{align}
\Delta_\theta J(\theta)&=E_{\tau\sim\pi_\theta}\left[\left(\sum_{t=1}^T \Delta_\theta \log \pi_\theta (a_t\mids_t) \right)\left(\sum_{t=1}^T r(s_t,a_t)\right)\right]\\
\Delta_\theta J(\theta)&\approx\frac{1}{N}\sum_{i=1}^N\left(\sum_{t=1}^T \Delta_\theta \log \pi_\theta (a_t\mids_t) \right)\left(\sum_{t=1}^T r(s_t,a_t)\right)\\
...
1. sample ${\tau^i}$ from $\pi_\theta(s_t\mids_t)$ (run the policy)
2. $\Delta_\theta J(\theta)\approx\sum_{i}^N\left(\sum_{t} \Delta_\theta \log \pi_\theta (a_t\mids_t) \right)\left(\sum_{t} r(s_t,a_t)\right)$
...
$$
\begin{align}
\Delta_\theta J(\theta)&\approx\frac{1}{N}\sum_{i=1}^N\left(\sum_{t=1}^T \Delta_\theta \log \pi_\theta (a_{i,t}\mids_{i,t}) \right)\left(\sum_{t=1}^T r(s_{i,t},a_{i,t})\right)\\
&\approx\frac{1}{N}\sum_{i=1}^N\sum_{t=1}^T \Delta_\theta \log \pi_\theta (a_{i,t}\mids_{i,t}) \left(\sum_{t'=t}^T r(s_{i,t’},a_{i,t'})\right)\\
&=\frac{1}{N}\sum_{i=1}^N\sum_{t=1}^T \Delta_\theta \log \pi_\theta (a_{i,t}\mids_{i,t})\hat{Q}_{i,t}
\end{align}
$$
...
$$
\pi_\theta(\tau)=p(s_1)\prod_{t=1}^T\pi_\theta(a_t\mids_t)p(s_{t+1}\mids_t,a_t)\\
\frac{\pi_\theta(\tau)}{\bar{\pi}(\tau)}=\frac{p(s_1)\prod_{t=1}^T\pi_\theta(a_t\mids_t)p(s_{t+1}\mids_t,a_t)}{p(s_1)\prod_{t=1}^T \bar{\pi}(a_t\mids_t)p(s_{t+1}\mids_t,a_t)}=\frac{\prod_{t=1}^T\pi_\theta(a_t\mids_t)}{\prod_{t=1}^T \bar{\pi}(a_t\mids_t)}
$$
...
&=E_{\tau\sim\pi_\theta}\left[\left(\prod_{t=1}^T\frac{\pi_{\theta'}(a_t\mids_t)}{\pi_\theta(a_t\mids_t)}\right)\left(\sum_{t=1}^T \Delta_{\theta'} \log \pi_{\theta'} (a_t\mids_t) \right)\left(\sum_{t=1}^T r(s_t,a_t)\right)\right]\\
&=E_{\tau\sim\pi_\theta}\left[\left(\sum_{t=1}^T \Delta_{\theta'} \log \pi_{\theta'} (a_t\mids_t) \right)\left(\prod_{t'=1}^t\frac{\pi_{\theta'}(a_{t'}\mids_{t'})}{\pi_\theta(a_{t'}\mids_{t'})}\right)\left(\sum_{t'=t}^T r(s_{t'},a_{t'})\left(\prod_{t''=t}^{T}\frac{\pi_{\theta'}(a_{t''}\mids_{t''})}{\pi_\theta(a_{t''}\mids_{t''})}\right)\right)\right]
\end{align}
$$
...
J(\theta')&=E_{s_t\sim p_\theta(s_t)}\left[\cancel{\frac{p_{\theta'}(s_t)}{p_{\theta}(s_t)}}E_{a_t\sim \pi(a_t,s_t)}\left[\frac{\pi_{\theta'}(a_t\mids_t)}{\pi_{\theta}(a_t\mids_t)}r(s_t,a_t)\right]\right]
\end{align}
$$
...
$$
\bar{J}(\theta)=\frac{1}{N}\sum_{i=1}^N\sum_{t=1}^T \log \pi_\theta (a_{i,t}\mids_{i,t})\hat{Q}_{i,t}
$$
...
$$
\hat{Q}_{i,t}\approx \sum_{t'=t}^T E_{\pi_\theta}[r(s_{t'},a_{t'})\mids_t,a_t]
$$

And we define

$$
\hat{Q}_{i,t}= \sum_{t'=t}^T E_{\pi_\theta}[r(s_{t'},a_{t'})\mids_t,a_t]\\
V(s_t)=E_{a_t\sim\pi(s_t\mids_t)}[Q(s_t,a_t)]\\
$$

then

$$
\Delta_\theta J(\theta)\approx\frac{1}{N}\sum_{i=1}^N\sum_{t=1}^T \Delta_\theta \log \pi_\theta (a_{i,t}\mids_{i,t})(Q(s_{i,t}, a_{i,t})-V(s_{i,t}))
$$
...
\Delta_\theta J(\theta)\approx\frac{1}{N}\sum_{i=1}^N\sum_{t=1}^T \Delta_\theta \log \pi_\theta (a_{i,t}\mids_{i,t})A^\pi(s_t,a_t)
$$
...
$$
Q^\pi(s_t,a_t)=r(s_t,a_t)+E_{s_{t+1}\sim p(s_{t+1}\mids_t,a_t})[V^\pi(s_{t+1})]
$$
...
$$
V^\pi(s_t)=\sum_{t'=t}^T E_{\pi_\theta}[r(s_{t'},a_{t'})\mids_t]\\
...
$$
y_{i,t}=\sum_{t'=t}^T E_{\pi_\theta}[r(s_{t'},a_{t'})\mids_t]\approx r(s_{s_{i,t}},a_{i,t})+V^\pi(s_{i,t+1})\approx r(s_{s_{i,t}},a_{i,t})+\hat{V^\pi_\phi}(s_{i,t+1})
$$
...
1. sample ${s_i,a_i}$ from $\pi_\theta(a\mids)$ 
2. fit $\hat{V_\phi^\pi}(s)$ to sampled reward sums
3. evaluate $\hat{A}^\pi(s_i,a_i)=r(s_i,a_i)+\hat{V}_\phi^\pi(s'_i)-\hat{V}_\phi^\pi(s_i)$ 
4. $\Delta_\theta J(\theta)\approx \sum_i \Delta_\theta \log \pi_\theta (a_{i}\mids_{i})\hat{A}^\pi(s_i,a_i)$ 
5. $\theta \gets \theta+\alpha\Delta_\theta J(\theta)$

$$
V^\pi(s_{i,t})=\sum_{t'=t}^TE_{\pi_\theta}[r(s_{t'},a_{t'})\mids_{i,t}]\\
...
$$
\Delta_\theta J(\theta)=\frac{1}{N}\sum_{i=1}^N\sum_{t=1}^T \Delta_\theta \log \pi_\theta (a_{i,t}\mids_{i,t})\left(\sum_{t'=t}^T\gamma^{t'-t}r(s_{i,t'},a_{i,t'})\right)
$$

Online actor-critic algorithm(can apply to every single step):

1. take action $a\sim\pi_\theta(a\mids)$, get $(s,a,s',r)$
...
4. $\Delta_\theta J(\theta)\approx \Delta_\theta \log \pi_\theta (a\mids)\hat{A}^\pi(s,a)$ 
5. $\theta \gets \theta+\alpha\Delta_\theta J(\theta)$
...
$$
\Delta_\theta J(\theta)\approx\frac{1}{N}\sum_{i=1}^N\sum_{t=1}^T \Delta_\theta \log \pi_\theta (a_{i,t}\mids_{i,t})\left(\sum_{t'=t}^T\gamma^{t'-t}r(s_{i,t'},a_{i,t'})-b\right)
$$

Actor-critic

$$
\Delta_\theta J(\theta)\approx\frac{1}{N}\sum_{i=1}^N\sum_{t=1}^T \Delta_\theta \log \pi_\theta (a_{i,t}\mids_{i,t})\left(r(s_i,a_i)+\hat{V}_\phi^\pi(s'_i)-\hat{V}_\phi^\pi(s_i)\right)
$$
...
$$
\Delta_\theta J(\theta)\approx\frac{1}{N}\sum_{i=1}^N\sum_{t=1}^T \Delta_\theta \log \pi_\theta (a_{i,t}\mids_{i,t})\left(\sum_{t'=t}^{\infty}\gamma^{t'-t}r(s_{i,t'},a_{i,t'})-\hat{V}_\phi^\pi(s_{i,t})\right)
$$
...
$$
\pi'(a_t\mids_t)=\begin{cases}1, &if \quad a_=\underset{a_t}{\arg\max}A^\pi(s_t,a_t)  \cr 0, &otherwise\end{cases}
$$

> this at least as good as any $a_t \sim \pi(a_t,\mids_t)$
...
assume we know $p(s'\mids,a)$ and s and a are both discrete (and small)

bootstrapped update:

$$
V^\pi(s) \gets E_{a\sim\pi(a\mids)}[r(s,a)+\gamma E_{s'\sim p(s'\mids,a)}[V^\pi(s')]]
$$

with deterministic policy $\pi(s)=a$, we have

$$
V^\pi(s) \gets r(s,\pi(s))+\gamma E_{s'\sim p(s'\mids,\pi(s))}[V^\pi(s')]
$$
...
1. set $y_i \gets \max_{a_i}(r(s_i\mida_i)+\gamma E[V_\phi(s'_i)])$
...
$$
Q^\pi(s,a) \gets r(s,a)+\gamma E_{s'\sim p(s'\mids,a)}[Q^\pi(s',\pi(s'))]
$$
...
3. set $\phi \gets \arg min_\phi \frac{1}{2}\sum_i\\midQ_\phi(s_i,a_i)-y_i\\mid^2$ repeat step 2,3 k times and then return step 1
...
1. epsilon-greedy
   ...
   $$
   \pi(a_t\mids_t)=\begin{cases}1-\epsilon , &if a_t={\arg\max}Q_\phi(s_t,a_t)  
   \cr \epsilon(\mid\mathcal{A}\mid-1), &otherwise\end{cases}
   $$

2. Boltzmann exploration

   $$\pi(a_t\mids_t) \propto \exp(Q_\phi(s_t,a_t))$$ 
...
1. save target network parameters: $\phi' \gets \phi$
   1. collect dataset ${(s_i,a_i,s_i',t_i)}$ using some policy, **add** it to $\mathcal{B}$ , do this N times
      1. sample a batch $(s_i,a_i,s_i',r_i)$ from $\mathcal{B} $ 
      2. $\phi \gets \arg min_\phi \frac{1}{2}\sum_i\\midQ_\phi(s_i,a_i)- [r(s_i,a_i) +\gamma \max_{a_i'}Q_{\phi'}(s_i',a_i')]\\mid^2$ , do 2,3 k times
...
- Bellman error gradients can be big;clip gradients or use Huber loss instead of square error
  ...
  $$
  \pi(a_t\mids_t)=\begin{cases}x^2/2 , &\text{if} \;\midx\mid\le\delta 
  \cr \delta\midx\mid-\delta^2/2, &\text{otherwise}\end{cases}
  $$
...
1. sample $\{\tau^i\}$ from $\pi_\theta(s_t\mids_t)$ (run the policy)
2. $\Delta_\theta J(\theta)\approx\sum_{i}\left(\sum_{t=1}^T \Delta_\theta \log \pi_\theta (a_t^i\mids_t^i) \left(\sum_{t'=t}^T r(s_{t'},a_{t'})\right)\right)$
3. $\theta \gets\theta+\alpha\Delta_\theta J(\theta)$
...
$$
\begin{align}
E_{\tau \sim p_{\theta'}(\tau)}\left[\sum_{t=0}^{\infty}\gamma^tA^{\pi_\theta}(s_t,a_t)\right]&=\sum_{t=0}^{\infty}E_{s_t \sim p_{\theta'}(s_t)}\left[E_{a_t \sim \pi_{\theta'}(a_t\mids_t)}\left[\gamma^tA^{\pi_\theta}(s_t,a_t)\right]\right]\\
&=\sum_{t=0}^{\infty}E_{s_t \sim p_{\theta'}(s_t)}\left[E_{a_t \sim \pi_{\theta}(a_t\mids_t)}\left[\frac{\pi_{\theta'}(a_t\mids_t)}{\pi_{\theta}(a_t\mids_t)}\gamma^tA^{\pi_\theta}(s_t,a_t)\right]\right]
\end{align}
$$
...
$\pi_{\theta'}$ if close to $\pi_\theta$ if $\mid\pi_{\theta'(a_t\mids_t)}-\pi_\theta(a_t\mids_t)\mid\le\epsilon$ for all $s_t$ 

$\midp_{\theta'}(s_t)-p_\theta(s_t)\mid\le 2\epsilon t$
...
$$
\begin{align}
E_{p_{\theta'}}[f(s_t)]=\sum_{s_t}p_{\theta'}(s_t)f(s_t)&\ge\sum_{s_t}p_\theta(s_t)f(s_t)-\midp_{\theta'}(s_t)-p_\theta(s_t)\mid\max_{s_t}f(s_t)\\
...
$$
\sum_t E_{s_t \sim p_{\theta'}(s_t)}\left[E_{a_t \sim \pi_{\theta}(a_t\mids_t)}\left[\frac{\pi_{\theta'}(a_t\mids_t)}{\pi_{\theta}(a_t\mids_t)}\gamma^tA^{\pi_\theta}(s_t,a_t)\right]\right]\\
\ge\sum_t E_{s_t \sim p_{\theta}(s_t)}\left[E_{a_t \sim \pi_{\theta}(a_t\mids_t)}\left[\frac{\pi_{\theta'}(a_t\mids_t)}{\pi_{\theta}(a_t\mids_t)}\gamma^tA^{\pi_\theta}(s_t,a_t)\right]\right]-\sum_t 2\epsilon t C
$$
...
$$
\theta' \gets \arg \max_{\theta'}\sum_t E_{s_t \sim p_{\theta}(s_t)}\left[E_{a_t \sim \pi_{\theta}(a_t\mids_t)}\left[\frac{\pi_{\theta'}(a_t\mids_t)}{\pi_{\theta}(a_t\mids_t)}\gamma^tA^{\pi_\theta}(s_t,a_t)\right]\right]\\
\text{such that}\:\:\mid\pi_{\theta'(a_t\mids_t)}-\pi_\theta(a_t\mids_t)\mid\le\epsilon
$$

For **small enough** $\epsilon$, this is **guaranteed to improve** $J(\theta')-J(\theta)$ 

A more convenient bound is using KL divergence: $\mid\pi_{\theta'(a_t\mids_t)}-\pi_\theta(a_t\mids_t)\mid\le \sqrt{\frac{1}{2}D_{KL}(\pi_{\theta'}(a_t\mids_t)\\mid\pi(a_t\mids_t))}$

$\Rightarrow D_{KL}(\pi_{\theta'}(a_t\mids_t)\\mid\pi(a_t\mids_t)$ bounds state marginal difference, where

$$
D_{KL}(p_1(s)\\midp_2(x))=E_{x\sim p_1(x)}\left[ \log \frac{p_1(x)}{p_2(x)}\right]
$$
...
$$
\theta' \gets \arg \max_{\theta'}\sum_t E_{s_t \sim p_{\theta}(s_t)}\left[E_{a_t \sim \pi_{\theta}(a_t\mids_t)}\left[\frac{\pi_{\theta'}(a_t\mids_t)}{\pi_{\theta}(a_t\mids_t)}\gamma^tA^{\pi_\theta}(s_t,a_t)\right]\right]\\\text{such that}\:\:D_{KL}(\pi_{\theta'}(a_t\mids_t)\\mid\pi(a_t\mids_t))\le\epsilon
$$
...
$$
\mathcal{L}(\theta',\lambda)=\sum_tE_{s_t \sim p_{\theta}(s_t)}\left[E_{a_t \sim \pi_{\theta}(a_t\mids_t)}\left[\frac{\pi_{\theta'}(a_t\mids_t)}{\pi_{\theta}(a_t\mids_t)}\gamma^tA^{\pi_\theta}(s_t,a_t)\right]\right]-\lambda(D_{KL}(\pi_{\theta'}(a_t\mids_t)\\mid\pi(a_t\mids_t))-\epsilon)
$$

1. Maximize $\mathcal{L}(\theta', \lambda)$ with respect to $\theta$   
2. $\lambda \gets + \alpha(D_{KL}(\pi_{\theta'}(a_t\mids_t)\\mid\pi(a_t\mids_t))-\epsilon)$
...
$$
\begin{align}
\bar{A}(\theta')&=\sum_t E_{s_t \sim p_{\theta}(s_t)}\left[E_{a_t \sim \pi_{\theta}(a_t\mids_t)}\left[\frac{\pi_{\theta'}(a_t\mids_t)}{\pi_{\theta}(a_t\mids_t)}\gamma^tA^{\pi_\theta}(s_t,a_t)\right]\right]\\
\bar{A}(\theta)&=\sum_t E_{s_t \sim p_{\theta}(s_t)}\left[E_{a_t \sim \pi_{\theta}(a_t\mids_t)}\left[\gamma^tA^{\pi_\theta}(s_t,a_t)\right]\right]
\end{align}
$$
...
\text{such that}\:\:D_{KL}(\pi_{\theta'}(a_t\mids_t)\\mid\pi(a_t\mids_t))\le\epsilon
$$
...
$$
\begin{align}
\Delta_{\theta'}\bar A(\theta')&=\sum_tE_{s_t \sim p_{\theta}(s_t)}\left[E_{a_t \sim \pi_{\theta}(a_t\mids_t)}\left[\frac{\pi_{\theta'}(a_t\mids_t)}{\pi_{\theta}(a_t\mids_t)}\gamma^t \Delta_{\theta'}\log{\pi_{\theta'}(a_t\mids_t)}A^{\pi_\theta}(s_t,a_t)\right]\right]\\
\Delta_{\theta}\bar A(\theta)&=\sum_tE_{s_t \sim p_{\theta}(s_t)}\left[E_{a_t \sim \pi_{\theta}(a_t\mids_t)}\left[\gamma^t \Delta_{\theta}\log{\pi_{\theta}(a_t\mids_t)}A^{\pi_\theta}(s_t,a_t)\right]\right]=\Delta_\theta J(\theta)
\end{align}
$$
...
by updating like $\theta'=\theta'+\sqrt{\frac{\epsilon}{\\mid\Delta_\theta J(\theta)\\mid^2}}\Delta_\theta J(\theta)$, this is what actually gradient ascent(policy gradient) doing.
...
$$
\pmb{F}=E_{\pi_\theta}[\Delta_{\theta}\log\pi_\theta(a\mids)\Delta_\theta\log \pi_\theta(a\mids)^T]
$$
...
In model-free RL, we do not know $p(s_{t+1}\mids_t,a_t)$. 
...
$$
p_\theta(s_1,...,s_T\mida_1,...,a_T)=p(s_1)\prod_{t=1}^Tp(s_{t+1}\mids_t,a_t)\\
a_1,...,a_T=\arg\max_{a_1,...,a_T}E\left[\sum_{t=1}^Tr(s_t,a_t)\mida_1,...,a_T\right]
$$
...
$$
p_\theta(s_1,a_1,...,s_T,a_T)=p(s_1)\prod_{t=1}^T\pi(a_t\mids_t)p(s_{t+1}\mids_t,a_t)\\
...
f(x_t,u_t)=F_t\begin{bmatrix}
           ...
           u_{t} \\
         \end{bmatrix}+f_t\\
x_{t-1}\sim p(x_{t+1}\midx_t,u_t)\\
 p(x_{t+1}\midx_t,u_t)=\mathcal{N}\left(F_t\begin{bmatrix}
           x_{t} \\
           u_{t} \\
...
>
> (or $p(s_{t+1}\mids_t,a_t)$ in stochastic case)

model-based reinforcement learning **version 0.5**:

1. run base policy $\pi_0(a_t\mids_t)$ (e.g., random policy) to collect $\mathcal{D}={(s,a,s')_i}$
2. learning dynamics model $f(s,a)$ to minimize $\sum_i\\midf(s_i,a_i)-s_i'\\mid^2$
...
$$
\int p(s_{t+1}\mids_t,a_t,\theta)p(\theta\mid\mathcal{D})d\theta
$$
...
\int p(s_{t+1}\mids_t,a_t,\theta)p(\theta\mid\mathcal{D})d\theta\approx\frac{1}{N}\sum_ip(s_{t+1}\mids_t,a_t,\theta_i)
$$
...
1. sample $\theta\sim p(\theta\mid\mathcal{D})$
2. at each time step $t$, sample $s_{t+1}\sim p(s_{t+1}\mids_t,a_t,\theta)$
...
$$
\max_\phi\frac{1}{N}\sum_{i=1}^N\sum_{t=1}^TE[\log p_\phi(s_{t+1,i}\mids_{t,i},a_{t,i})+\log p_\phi(o_{t,i}\mids_{t,i})]
$$

learn *approximate* posterior $q_\psi(s_t\mido_{1:t},a_{1:t})$ 

other choices:

- $q_\psi(s_t,s_{t+1}\mido_{1:t},a_{1:t})$
- $q_\psi(s_t\mido_t)$

here we only estimate $q_\psi(s_t\mido_t)$

assume that  $q_\psi(s_t\mido_t)$ is *deterministic*
...
$$
q_\psi(s_t\mido_t)=\delta(s_t=g_\psi(o_t))\Rightarrow s_t=g_\psi(o_t)
$$

and maybe the reward also need to learn.

$$
\max_{\phi,\psi}\frac{1}{N}\sum_{i=1}^N\sum_{t=1}^TE[\log p_\phi(g_\psi(o_{t+1,i})\midg_\psi(o_{t,i}),a_{t,i})+\log p_\phi(o_{t,i}\midg_\psi (o_{t,i})]\\
\max_{\phi,\psi}\frac{1}{N}\sum_{i=1}^N\sum_{t=1}^TE[\log p_\phi(g_\psi(o_{t+1,i})\midg_\psi(o_{t,i}),a_{t,i})+\log p_\phi(o_{t,i}\midg_\psi (o_{t,i})+log p_\phi(r_{t,i}\midg_\psi(o_{t,i}))]
$$

Model-based RL with latent space models

1. run base policy $\pi_0(o_t\mida_t)$ (e.g., random policy) to collect $\mathcal{D}={(o,a,o')_i}$
2. learning $p_\psi(s_{t+1}\mids_t,a_t), p_\psi(r_t\mids_t), p(o_t\mids_t), g_\psi(o_t)$
...
directly learn $p(o_{t+1}\mido_t,a_t)$ 
...
1. run base policy $\pi_0(a_t\mids_t)$ (e.g., random policy) to collect $\mathcal{D}={(s,a,s')_i}$
2. learning dynamics model $f(s,a)$ to minimize $\sum_i\\midf(s_i,a_i)-s_i'\\mid^2$
3. back-propagate through $f(s,a)$ into the policy to optimize $\pi_\theta(a_t\mids_t)$
4. run $\pi_\theta (a_t\mids_t)$, appending the visited tuples $(s,a,s')$ to $\mathcal{D}$ , repeat steps 2~4
...
$$
\bar{\mathcal{L}}(x,\lambda)=f(x)+\lambda C(x)+\rho\\midC(x)\\mid^2
$$
...
- supervised objective for $\pi_\theta(u_t\midx_t)$
...
$$
\tilde{c}_{k+1,i}(x_t,u_t)=c(x_t,u_t)+\lambda_{k+1,i}\log \pi_\theta (u_t\midx_t)
$$
...
$$
\min_{p,\theta}E_{\tau\sim p(\tau)}{c(\tau)}\:\:\text{s.t.}\:\:p(u_t\midx_t)=\pi_\theta(u_t\midx_t)\\
p(u_t\midx_t)=\mathcal{N}(K_t(x_t-\hat{x}_t)+k_t+\hat{u}_t,\Sigma_t)
$$
...
$$
\min_{p,\theta}E_{\tau\sim p(\tau)}[{c(\tau)}]\:\:\text{s.t.}\:\:p(u_t\midx_t)=\pi_\theta(u_t\mido_t)\
$$
...
3. execute action $s_t\sim\pi(a_t\mids_t)$ (not MCTS action!). repeat 1~3 N times
...
1. train $\pi_\theta(u_t\mido_t)$ from labeled data $\mathcal{D}={o_1,u_1,...,o_N,u_N}$
2. run $\hat{\pi}(u_t\mido_t)$ to get dataset $\mathcal{D}_\pi={o_1,...,o_M}$
3. Ask computer to label $\mathcal{D_\pi}$ with actions $u_t$
4. Aggregate: $\mathcal{D}\gets\mathcal{D}\cup\mathcal{D}_\pi$

**Simple** stochastic policy: $\hat{\pi}(u_t\midx_t)=\mathcal{N}(K_tx_t+k_t, \Sigma_{u_t})$

$$
\hat{\pi}(u_t\midx_t)=\arg\min_{\hat{\pi}}\sum_{t'=t}^TE_{\hat{\pi}}[c(x_{t'},u_{t'})]+\lambda D_{KL}(\hat{\pi}(u_t\midx_t)\\mid\pi_\theta(u_t\mido_t))
$$
...
3. update model $\hat{p}(s'\mids,a)$ and $\hat{r}(s,a)$ using $(s,a,s')$
...
1. given state $s$, pick action $a$ using exploration policy
2. learn model $\hat{p}(s'\mids,a)$ (and optionally, $\hat{r}(s,a)$) 
3. repeat K times:
   4. sample $s\sim\mathcal{B}$  from buffer
   5. choose action a (from ​$\mathcal{B}$, from $\pi$, or random)
   6. simulate $s'\sim\hat{p}(s'\mids,a)$ (and $r=\hat{r}(s,a)$)
...
$$
p(x)=\sum_zp(x\midz)p(z)\\
p(y\midx)=\sum_zp(y\midx,z)p(z)
$$
...
$$
p(x\midz)=\mathcal{N}(\mu_{nn},\sigma_{nn}(z))\\
...
the model: $p(x)=\int p(x\midz)p(z)dz$

maximum likelihood fit: $\theta\gets\arg\max_\theta \frac{1}{N} \sum_i \log \left(\int p(x\midz)p(z)dz\right)$

Estimating the log-likelihood

alternative: *expected* log-likelihood: $\theta\gets\arg\max_\theta \frac{1}{N} \sum_i E_{z\sim p(z\midx_i)} \log p_\theta(x_i,z)$

##### The variational approximation

approximate $p(z\midx_i)$ with $q_i(z)=\mathcal{N}(\mu_i,\sigma_i)$ 

$$
\begin{align}
\log p(x_i) &= \log \int_z p(x_i\midz)p(z)\\
&=\log \int_z p(x_i\midz)p(z)\frac{q_i(z)}{q_i(z)}\\
&=\log E_{z\sim q_i(z)}\left[\frac{p(x_i\midz)p(z)}{q_i(z)}\right]\\
&\ge E_{z \sim q_i(z)}\left[\log \frac{p(x_i\midz)p(z)}{q_i(z)}\right]\\
&= E_{z \sim q_i(z)}[\log p(x_i\midz)+\log p(z)]- E_{z \sim q_i(z)}[\log q_i(z)]\\
&= E_{z \sim q_i(z)}[\log p(x_i\midz)+\log p(z)]+ \mathcal{H}(q_i)
\end{align}
$$
...
KL Divergence: $D_{KL}(q\mid\midp)=E_{x \sim q(x)}\left[\log \frac{q(x)}{p(x)}\right]=E_{x \sim q(x)}[\log q(x)]- E_{x \sim q(x)}[\log p(x)] =-E_{x \sim q(x)}[\log p(x)]- \mathcal{H}(q)$

further analysis

Unable to render expression.
$$
\log p(x_i) = E_{z \sim q_i(z)}[\log p(x_i\midz)+\log p(z)]+ \mathcal{H}(q_i)=\mathcal{L}_i(q,p_i)
$$

so what makes a good $q_i(z)$? 

intuition: $q_i(z)$ should approximate $p(z\midx_i)$

why?

$$
\begin{align}
D_{KL}(q_i(z_i\mid\midp(z\midx_i)))&=E_{z\sim q_i(z)}\left[\log \frac{q_i(z)}{p(z\midx_i)}\right]\\
&=E_{z \sim q_i(z)}\left[\log \frac{q_i(z)p(x_i)}{p(x_i,z)}\right]\\
&=-E_{z\sim q_i(z)}[\log p(x_i\midz)+\log p(z)] +E_{z \sim q_i(z)}+E_{z\sim q_i(z)}[\log q_i(z)]+E_{z \sim q_i(z)}[\log p(x_i)]\\
&=-E_{z\sim q_i(z)}[\log p(x_i\midz)+\log p(z)] +E_{z \sim q_i(z)}-\mathcal{H}(q_i)\log p(x_i)\\
...
$$
\begin{align}
\log p(x_i)&=D_{KL}(q_i(z_i\mid\midp(z\midx_i)))+\mathcal{L}_i(p,q_i)\\
0 &\le D_{KL}(q_i(z_i\mid\midp(z\midx_i))) \\
\log p(x_i)&\ge \mathcal{L}_i(p,q_i)
\end{align}
$$

> So this  also prove that $\mathcal{L}_i(p,q_i)$ is low bound, and the KL Divergence is the bound gap, so  when $q_i(x)$ close to $p(z\midx_i)$, the bound will become tight.
>
...
$$
\mathcal{L}_i(q,p_i)=E_{z \sim q_i(z)}[\log p_\theta(x_i\midz)+\log p(z)]+ \mathcal{H}(q_i)
$$
...
​	$\Delta_\theta \mathcal{L}(p,q_i)\approx\Delta_\theta \log p_\theta(x_i\midz)$
...
every sample has a $\mu_i,\sigma_i$, when you have many samples , the total paramters are $\mid\theta\mid+(\mid\mu_i\mid+\mid\sigma_i\mid)*N$

intuition: $q_i(z)$ should approximate $p(z\midx_i)$

what if we learn a  *network* $q_i(z)=q(z\midx_i)\approx p(z\midx_i)$ ?

so we have two network: $p_\theta(x\midz)$ and $q_\phi(z\midx)$

### Amortized variational inference

$$
q_\phi(z\midx)=\mathcal{N}(\mu_\phi(x),\sigma_\phi(x))
$$

$$
\log p(x_i)\ge E_{z \sim q_\phi(z\midx_i)}[\log p_\theta(x_i\midz)+\log p(z)]+ \mathcal{H}(q_\phi(z\midx_i))=\mathcal{L}(q_\theta(x_i\midz),q_\phi(z\midx_i))
$$
...
​	calculate $\mathcal{L}(q_\theta(x_i\midz),q_\phi(z\midx_i))$

​	sample $z \sim q_\phi(z\midx_i)$
...
$$
\mathcal{L}_i=E_{z \sim q_\phi(z\midx_i)}[\log p_\theta(x_i\midz)+\log p(z)]+ \mathcal{H}(q_\phi(z\midx_i))\\
J(\phi)= E_{z \sim q_\phi(z\midx_i)}[r(x_i,z)]\\
J(\phi)\approx \frac{1}{M}\sum_j\Delta_\phi \log q_\phi(z_j\midx_i)r(x_i,z_j)
$$
...
\begin{align}
\mathcal{L}_i&=E_{z \sim q_\phi(z\midx_i)}[\log p_\theta(x_i\midz)+\log p(z)]+ \mathcal{H}(q_\phi(z\midx_i))\\
&=E_{z \sim q_\phi(z\midx_i)}[\log p_\theta(x_i\midz)]+E_{z \sim q_\phi(z\midx_i)}[\log p(z)]+ \mathcal{H}(q_\phi(z\midx_i))\\
&=E_{z \sim q_\phi(z\midx_i)}[\log p_\theta(x_i\midz)]-D_{KL}(q_\phi(z\midx_i\mid\midp(z)))\\
&=E_{\epsilon \sim \mathcal{N}(0,1)}[p_\theta(x_i,\mu_\phi(x_i)+\epsilon\sigma_\phi(x_i))]-D_{KL}(q_\phi(z\midx_i\mid\midp(z)))\\
&\approx \log p_\theta(x_i,\mu_\phi(x_i)+\epsilon\sigma_\phi(x_i))-D_{KL}(q_\phi(z\midx_i\mid\midp(z)))
\end{align}
$$

#### Re-parameterization trick vs. policy gradient

- policy gradient     $J(\phi)\approx \frac{1}{M}\sum_j\Delta_\phi \log q_\phi(z_j\midx_i)r(x_i,z_j)$
...
Conditional models
$$
\mathcal{L}_i=E_{z \sim q_\phi(z\midx_i,y_i)}[\log p_\theta(y_i\midx_i,z)+\log p(z\midx_i)]+ \mathcal{H}(q_\phi(z\midx_i,y_i))
$$
...
>
> we want to represent the distribution of a object(by neural network) and make it has ability to  generalize the feature of the object (multi-modal). To achieve this, the random variables($z$) are used as input of model, but what kind of  random variances can achieve this? the mathematic proof shows that the distribution of random variance should approximate $p(z\midx_i)$, this is kind of compress sensing, where $z$ is  the latent variable. Finally, this is used to do variational auto encoder (VAE).
>
...
$$
p(\tau\mid\mathcal{O}_{1:T})\:\:\:\;\;\;p(\mathcal{O}_t\mids_t,a_t)=\exp(r(s_t,a_t))\\
p(\tau\mid\mathcal{O}_{1:T})=\frac{p(\tau,\mathcal{O}_{1:T})}{p(\mathcal{O}_{1:T})}\propto p(\tau)\prod\exp(r(s_t,a_t))=p(\tau)\exp\left(\sum_tr(s_t,a_t\right)
$$
...
$$
\begin{align}
\beta_t(s_t,a_t)&=p(\mathcal{O}_{t:T}\mids_t,a_t)\\
&=\int p(\mathcal{O}_{t:T},s_{t+1}\mids_t,a_t)ds_{t+1}\\
&=\int p(\mathcal{O}_{t+1:T}\mids_{t+1}) p(s_{t+1}\mids_t,a_t)p(\mathcal{O}_t\mids_t,a_t)ds_{t+1}\\
\end{align}
$$

$$
p(\mathcal{O}_{t+1:T}\mids_{t+1})=\beta_{t+1}(s_{t+1})=\int p(\mathcal{O}_{t+1:T}\mids_{t+1,}a_{t+1})p(a_{t+1}\mids_{t+1})d a_{t+1}\\
=\int \beta_t(s_{t+1,}a_{t+1})p(a_{t+1}\mids_{t+1})d a_{t+1}
$$
...
> $$
> \begin{align}
> \beta_t(s_t,a_t)&=p(\mathcal{O}_t\mids_t,a_t)E_{s_{t+1}\sim p(s_{t+1}\mids_t,a_t)}[\beta_{t+1}(s_{t+1})]\\
> \beta_t(s_t)&=E_{s_t\sim p(a_t\mids_t)}[\beta_t(s_t,a_t)]
> \end{align}
> $$
...
$$
\begin{align*}
p(a_t\mids_t,\mathcal{O}_1:T)&=\pi(a_t\mids_t)\\
&=p(a_t\mids_t,\mathcal{O}_t:T)\\
&=\frac{p(a_t,a_t\mid\mathcal{O}_{t:T})}{p(s_t\mid\mathcal{O}_{t:T})}\\
&=\frac{p(\mathcal{O}_{t:T}\mida_t,s_t)p(a_t,s_t)/p(a_t,s_t)}{p(\mathcal{O}_{t:T}\mids_t)p(s_t)/p(\mathcal{O}_{t:T})}\\
&=\frac{p(\mathcal{O}_{t:T}\mida_t,s_t)}{p(\mathcal{O}_{t:T}\mids_t)}\frac{p(a_t,s_t)}{p(s_t)}\\
&=\frac{\beta_t(s_t,a_t)}{\beta_t(s_t)}p(a_t\mids_t)\\
...
$$
\pi(a_t\mids_t)=\frac{\beta_t(s_t,a_t)}{\beta_t(s_t)}\\
...
$$
\pi(a_t\mids_t)=\exp(Q_t(s_t,a_t)-V_t(s_t))=\exp(A_t(s_t,a_t))
$$

with temperature: $\pi(a_t\mids_t)=\exp(\frac{1}{\alpha}Q_t(s_t,a_t)-\frac{1}{\alpha}V_t(s_t))=\exp(\frac{1}{\alpha}A_t(s_t,a_t))$ if $\alpha$ is near zero, the max action will dominate, so it's  more like deterministic, and $\alpha$ near 1 is more stochastic.
...
&=\int p(s_t\mids_{t-1},a_{t-1},\mathcal{O}_{1:t-1})p(a_{t-1}\mids_{t-1},\mathcal{O}_{1:t-1})p(s_{t-1\mid\mathcal{O}_{1:t-1}})ds_{t-1}da_{t-1}\\
&=\int p(s_t\mids_{t-1},a_{t-1})p(a_{t-1}\mids_{t-1},\mathcal{O}_{1:t-1})p(s_{t-1\mid\mathcal{O}_{1:t-1}})ds_{t-1}da_{t-1}
\end{align*}
$$

$$
\begin{align*}
p(s_t\mids_{t-1},a_{t-1})p(a_{t-1}\mids_{t-1},\mathcal{O}_{1:t-1})
&=\frac{p(\mathcal{O}_{t-1}\mids_{t-1},a_{t-1})p(a_{t-1}\mids_{t-1})}{p(\mathcal{O}_{t-1}\mids_{t-1})}\frac{p(\mathcal{O}_{t-1}\mids_{t-1})p(s_{t-1}\mid\mathcal{O}_{1:t-2})}{p(\mathcal{O}_{t-1}\mid\mathcal{O}_{1:t-2})}\\
&=\frac{p(\mathcal{O}_{t-1}\mids_{t-1},a_{t-1})p(a_{t-1}\mids_{t-1})}{1}\frac{\alpha_{t-1}(s_{t-1})}{p(\mathcal{O}_{t-1}\mid\mathcal{O}_{1:t-2})}
\end{align*}
$$

what if we want $p(s_t\mid\mathcal{O}_{1:T})$? 

$$
\begin{align*}
p(s_t\mid\mathcal{O}_{1:T})&=\frac{p(s_t,\mathcal{O}_{1:T})}{p(\mathcal{O}_{1:T})}\\
&=\frac{p(\mathcal{O}_{t:T}\mids_t)p(s_t,\mathcal{O}_{1:t-1})}{p(\mathcal{O}_{1:T})}\\
...
The inference problem: $p(s_{1:T},a_{1:T}\mid\mathcal{O}_{1:T})$

marginalizing and conditioning, we get: $p(a_t\mids_t,\mathcal{O}_{1:T})$ (the policy)

> "given that you obtained high reward, what was your action probability?"

marginalizing and conditioning, we get: $p(s_{t+1},\mids_t,a_t,\mathcal{O}_{1:T})\ne p(s_{t+1\mids_t,a_t})$ 
...
find anther distribution $q(s_{1:T},a_{1:T})$ that is close to $p(s_{1:T},a_{1:T}\mid\mathcal{O}_{1:T})$ but has dynamics $p(s_{t+1}\mids_t,a_t)$

Try variational inference!

let $\mathrm{x}=\mathcal{O}_{1:T}$ and $\mathrm{z}=(s_{1:T},s_{1:T})$  find $q(\mathrm{z})$ to approximate $p(\mathrm{z}\mid\mathrm{x})$

#### Control via variational inference

$$
q(s_{1:T},a_{1:T})=p(s_1)\prod_t p(s_{t+1}\mids_t,a_t)q(a_t\mids_t)
$$
...
\log p(\mathcal{O}_{1:T}) &\ge E_{s_{1:T},a_{1:T}\sim q}[\log p(s_1)+\sum_{t=1}^T\log p(s_{t+1\mids_t,a_t})+\sum_{t=1}^T\log p(\mathcal{O}_t\mids_t,a_t)\\-\log p(s_1)-&\sum_{t=1}^T\log p(s_{t+1}\mids_t,a_t)-\sum_{t=1}^T\log q(s_t\mids_t)]\\
&=E_{(s_{1:T},a_{1:T})\sim q}\left[\sum_tr(s_t,a_t)-\log q(a_t\mids_t)\right]\\
&=\sum_t E_{(s_t,a_t)\sim q}[r(s_t,a_t)+\mathcal{H}(q(a_t\mids_t))]
\end{align}
$$
...
$\pi(a\mids)=\exp(Q_\phi(s,a)-V(s))=\exp(A(s,a))$

> _Detailed timeline graphic available in the PDF notes._

#### Policy gradient with soft optimality

$\pi(a\mids)=\exp(Q_\phi(s,a)-V(s))$ optimizes $\sum_{\pi(s_t,a_t)}[r(s_t,a_t)]+E_{\pi(s_t)}[\mathcal{H}(\pi(a_t\mids_t))]$

**intuition:** $\pi(a\mids)\propto \exp(Q_\phi(s,a))$ when $\pi$ minimizes $D_{KL}(\pi(a\mids)\mid\mid\frac{1}{Z}\exp(Q(s,a))$
...
$$
J(\theta)=\sum_tE_{\pi(s_t,a_t)}[r(s_t,a_t)]+E_{\pi(s_t)}[\mathcal{H}(\pi(a\mids_t))]\\
=\sum_tE_{\pi(s_t,a_t)}[r(s_t,a_t)-\log \pi(a_t\mids_t)]\\
\log \pi(s_t\mids_t)=Q(s_t,a_t)-V(s_t)
$$

$$
\begin{align}
&\Delta_\theta\left[\sum_tE_{\pi(s_t,a_t)}[r(s_t,a_t)-\log \pi(a_t\mids_t)]\right]\\
&\approx \frac{1}{N}\sum_i\sum_t\Delta_\theta \log \pi(a_t\mids_t)\left(r(s_t,a_t)+\left(\sum_{t'=t+1}^Tr(s_{t'},s_{t'})-\log \pi(a_{t'}\mids_{t'})\right)-\log \pi(a_t\mids_t)-1\right)\\
...
(sometimes) transitions $p(s'\mids,a)$
...
… and then use it to learn $\pi*(a\mids)$

#### Learning the optimality variable

$p(\mathcal{O}_t\mids_t,a_t,\psi)=\exp(r_\psi(s_t,a_t))$

$p(\tau\mid\mathcal{O}_{1:T},\psi) \propto p(\tau)\exp\left(\sum_tr_\psi(s_t,a_t\right)$

given:

 	samples $\{\tau_i\}$ sampled from $\pi*(\tau)$

​	maximum likelihood learning:       $\max_\psi \frac{1}{N}\sum_{i-1}^N\log p(\tau_i\mid\mathcal{O}_{1:T},\psi)=\max_\psi \frac{1}{N}\sum_{i=1}^Nr_{\psi}(\tau_i)-\log Z$
...
$ p(s_t,a_t\mid\mathcal{O}_{1:T},\psi)=p(a_t\mids_t,\mathcal{O}_{1:T},\psi)p(s_t\mid\mathcal{O}_{1:T},\psi)$
...
instead of learn the $p(a_t\mids_t, \mathcal{O}_{1:T},\psi)$using any max-ant RL algorithm to converge then run this policy to sample ${\tau_j}$, can we just run one or several gradient step and  then sample? 
...
&=\frac{p(s_1)\prod_tp(s_{t+1}\mids_t,a_t)\exp(r_\psi(s_t,a_t))}{p(s_1)\prod_tp(s_{t+1}\mids_t,a_t)\pi(a_t\mids_t)}\\
&=\frac{\exp(\sum_tr_\psi(s_t,a_t))}{\prod_t\pi(a_t\mids_t)}
\end{align}
$$
...
&=\frac{\frac{1}{Z}\exp(r(\tau))}{\prod_t\pi_\theta(a_t\mids_t)+\frac{1}{Z}\exp(r(\tau))}\\
&=\frac{\frac{1}{Z}\exp(r(\tau))}{\prod_t\pi_\theta(a_t\mids_t)+\frac{1}{Z}\exp(r(\tau))}
\end{align}
$$
...
let $\mathcal{H}(\hat{p}(z)\midy)$ be the entropy of our $z$ estimate after observation $y$

Information gain is

$$
IG(z,y)=E_y[\mathcal{H}(\hat{p}(z))-\mathcal(\hat{p}(z)\midy)]\\
IG(z,y\mida)=E_y[\mathcal{H}(\hat{p}(z))-\mathcal(\hat{p}(z)\midy)\mida]
$$
...
- Heuristic estimation of counts via errors
  ...
  use $\xi(s,a)=\mid\mid\hat{f}_\theta(s,a)-f^*(s,a)\mid\mid^2$ as bonus
...
variational inference:
 ...
 IG can be equivalently written as $D_{KL}(p(z\midy)\mid\midp(z))$

learn about *transitions* $p_\theta(s_{t+1}\mids_t,a_t): z=\theta$ 

$y = (s_t,a_t,s_{t+1})$       $D_{KL}(p(\theta\midh,s_t,a_t,s_{t+1})\mid\midp(\theta\midh))$  where h is the history of all prior transitions

intuition: a transition is more informative if it causes belief over $\theta$ to change

idea: use variational inference to estimate $q(\theta\mid\phi)\approx p(\theta\midh)$

given new transition $(s,a,s')$, update $\phi$ to get $\phi'$

and use $D_{KL}(q(\theta\mid\phi)\mid\midq(\theta\mid\phi))$ as approximate bonus
...
2. initialize $\pi_\theta$ as $\max_\theta \sum_i \log \pi_\theta(a_i\mids_i)$
...
$$
\Delta J(\theta)=\sum_{\tau \in \mathcal{D}}\left[\sum_{t=1}^T\Delta_\theta\log\pi_\theta(a_t\mids_t)\left(\prod_{t'=1}^t\frac{\pi_\theta(a_{t'}\mids_{t'})}{q(a_{t'}\mids_{t'})}\right)\left(\sum_{t'=t}^Tr(s_{t'},a_{t'})\right)\right]
$$
...
How to pick $\pi_{new}(a\mids)$?

option 1: stay close to $\beta$

- e.g. $D_{KL}(\pi_{new}(.\mids)\mid\mid\beta(.\mids))\le \epsilon$
...
imitation objective: $\sum_{(s,a) \in \mathcal{D}_{demo}}\log \pi_\theta(a\mids)$

RL objective: $E_{\pi_\theta}[r(s,a)]$

hybrid objective: $E_{\pi_\theta} [r(s,a)]+\lambda \sum_{(s,a) \in \mathcal{D}_{demo}} \log \pi_\theta(a\mids)$
...
$$
\begin{align}
\mathcal{I}&=D_{KL}(p(x,y)\mid\midp(x)p(y))\\
&=E_{(x,y)\sim p(x,y)}\left[\log \frac{p(x,y)}{p(x)p(y)}\right]\\
&=\mathcal{H}(p(y))-\mathcal{H}(p(y\midx))
\end{align}
$$
...
empowerment: $\mathcal{I}(s_{t+1},a_t)=\mathcal{H}(s_{t+1})-\mathcal{H}(s_{t+1}\mida_t)$

### Learning without a reward function by reaching goals

one way is giving goal states, maybe using VAE to generate goals

1. Propose goal: $z_g \sim p(z)$, $x_g \sim p_\theta(x_g\midz_g)$
2. Attempt to reach goal using $\pi(a\midx,x_g)$, reach $\bar{x}$
3. Use data to update $\pi$
4. Use data to update $p_\theta(x_g\midz_g)$, $q_\phi(z_g\midx_g)$
...
$\pi(s\midS,G)$ trained to reach goal G as $\pi$ gets better, final state S gets close to G, 

that means $p(G\midS)$ becomes more deterministic!

so we actually doing this

$$
\max \mathcal{H}(p(G))-\mathcal(p(G\midS))=\max \mathcal{I}(S;G)
$$

### Learning diverse skills

$\pi(s\mids,z)$ and $z$ is task index.
...
$$
\pi(a\mids,z) =\arg \max_\pi \sum_z E_{s \sim \pi(s\midz)}[r(a,z)]$
$$
