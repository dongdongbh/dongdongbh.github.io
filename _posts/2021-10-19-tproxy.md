...
When you are using a proxy client/server, things might change.  In the following part of this section, I only show the DNS working with a proxy. The direct link is similar as the  normal http connection.
...
`dokodemo-door` is used for receive all traffic forward to V2ray.
