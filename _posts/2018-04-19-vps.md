...
2. try `wget http://www.google.com` to test. 
...
on ubuntu, the snap using `/etc/environment` as env variables, so you have to set proxy in it for snap, `sudo vim /etc/environment`, add
